...
const ManageQuestions = () => {
  ...
  const activeQuestions = userQuestions
  ...
  const handleRemove = (questionId: string) => {
    ...
    // Also remove custom question if it is one
    const isCustom = customQuestions.find(q => q.id === questionId);
    if (isCustom) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="px-5 pt-6 pb-4">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Meine Fragen</h1>
            <p className="text-sm text-muted-foreground">Definiere deine täglichen Check-in Fragen</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 space-y-2 pb-4">
        {activeQuestions.map((q) => (
          <div
            key={q.id}
            className="bg-card rounded-xl shadow-card p-3.5 flex items-center gap-3 border border-border/50"
          >
      ...
      <div className="px-5 pb-8 space-y-3">
        <button
          ...
          className="w-full p-3.5 rounded-xl border-2 border-dashed border-primary/40 bg-primary/5 flex items-center justify-center gap-2 text-primary font-semibold text-sm hover:bg-primary/10 transition-colors"
        >
        ...
        <button
          ...
          className="w-full p-3.5 rounded-xl border-2 border-dashed border-border flex items-center justify-center gap-2 text-muted-foreground font-semibold text-sm hover:bg-secondary transition-colors"
        >
        ...
        <div className="fixed inset-0 bg-foreground/40 z-50 flex items-end sm:items-center justify-center">
          <div className="bg-card w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl max-h-[85vh] flex flex-col animate-slide-in">
            <div className="flex items-center justify-between p-5 pb-3">
              <h2 className="text-lg font-bold text-foreground">Fragen-Bibliothek</h2>
              <button onClick={() => { setShowLibrary(false); setSearchQuery(''); }} className="p-1">
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-2">
              {libraryQuestions.map((q) => (
                ...
                >
                  ...
                  <button
                    onClick={() => handleAddFromLibrary(q.id)}
              ...
              {libraryQuestions.length === 0 && (
                <p className="text-center text-muted-foreground text-sm py-8">Keine weiteren Fragen verfügbar</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-foreground/40 z-50 flex items-end">
          <div className="bg-card w-full rounded-t-2xl p-6 animate-slide-in">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-foreground">Neue Frage</h2>
              ...
              {(['yesno', 'scale', 'freetext'] as QuestionType[]).map(type => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                    newType === type ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground'
                  }`}
                >
                  {getTypeLabel(type)}
                </button>
